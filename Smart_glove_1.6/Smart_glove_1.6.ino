...
void LECTURA(int x)
{
   d5[x]= digitalRead (D1)* 10000; // MEÑIQUE
   d4[x]= digitalRead (D2)* 1000; // ANULAR
   d3[x]= digitalRead (D3)* 100; // MAYOR
   d2[x]= digitalRead (D4)* 10; // INDICE
   d1[x]= digitalRead (D5)* 1; // GORDO
}
/*LO QUE HACE FILTRO ES BASICAMENTE FIJARSE SI UN DEDO ESTA BAJO, Y SI ESTA CONDICION SE REPITE 2 VECES SEGUIDAS EN EL FOR, 
  SE HACE UN BREAK Y SE IMPRIME ESA LETRA*/
void FILTRO()
{
 for (int x=0; x<5; x++)
 { 
  LECTURA(x);
   ...
   TOT[x]= d1[x]+d5[x]+d4[x]+d3[x]+d2[x]; //SUMA TODOS LOS DEDOS, PARA ASI TENER UN TOTAL POR PASADA (EN TOTAL SON 5 PASADAS DEL FOR)
   delay(10);
 }
}
/*LO QUE HACE ABC ES BUSCAR EL TOTAL OBTENIDO ANTERIORMENTE Y BUSCAR LA LETRA QUE LE CORRESPONDE SEGUN EL MODO EN EL QUE ESTÉ*/
int ABC(int Modo, int TOTAL)
{
  if(Modo==1)
  {
  switch(TOTAL)
  ...
  switch(TOTAL)
  {
    case 10: Serial.print("HOLA "); mySerial.write('a');break;
    case 11: Serial.print("CHAU "); mySerial.write('b');break;
    case 100: Serial.print("¿CÓMO ANDAS? "); mySerial.write('e');break;
    case 101: Serial.print("MI NOMBRE ES "); mySerial.write('f');break;
    case 110: Serial.print("¿CÓMO TE LLAMAS?"); mySerial.write('g');break;
    case 111: Serial.print("DOUUUUUUUUU"); mySerial.write('h');break;

    case 1000: Serial.print("M3"); mySerial.write('t');Modo=3; break;
    case 1101: Serial.print("M1"); mySerial.write('u');Modo=1; break;
    case 1110: Serial.print("M2"); mySerial.write('d');Modo=2; break;
    case 1111: Serial.print("*"); mySerial.write('*'); break;
 ...
 void setup() 
{
  ...
  Serial.begin(9600);
  mySerial.begin(9600);
  Modo=1; // se setea en 1 asi comienza en ese modo
}
...
void loop() 
{
  INICIALIZACION();
  FILTRO();
  TOTAL=TOT[0]+ TOT[1]+ TOT[2]+ TOT[3]+ TOT[4] ; //SE SUMAN TOTDOS LOS TOTALES DEL FOR Y SE OBTIENE UN NUMERO QUE INDICARA QUE LETRA SE QUIERE
  Modo= ABC(Modo,TOTAL);
  delay(500); //ES LA VELOCIDAD MAXIMA CON LA QUE EL USUARIO PODRÁ ESCRIBIR UNA LETRA
}
